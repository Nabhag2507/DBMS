...
SELECT FacultyName
FROM FACULTY
WHERE FacultyJoiningDate < '01-01-2015'
...
--without subquery
SELECT STUNAME,STUDENTS.StudentID,ENROLLMENT.StudentID
FROM STUDENTS LEFT JOIN ENROLLMENT
ON STUDENTS.StudentID = ENROLLMENT.StudentID
...
SELECT COUNT(EnrollmentID),CourseName
...
JOIN FACULTY ON FACULTY.FacultyID = COURSE_ASSIGNMENT.FacultyID
...
SELECT StuName,COUNT(EnrollmentID)
FROM STUDENTS JOIN ENROLLMENT
ON STUDENTS.StudentID = ENROLLMENT.StudentID
GROUP BY StuName
...
SELECT
	...
	ROUND(AVG(CAST(DATEDIFF(YEAR, FacultyJoiningDate, GETDATE()) AS FLOAT)), 2) AS AvgYearsExperience
FROM FACULTY
